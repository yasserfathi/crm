                ...
                <v-row>
                    ...
                    <v-col>
                        <v-sheet min-height="40vh" rounded="lg">
                            <v-container>
                                <v-data-table :headers="headers" :items="customers" item-value="profile" :items-per-page="5"
                                                ...
                                                <v-card>
                                                    ...
                                                    <v-card-text>
                                                        <v-container>
                                                            ...
                                                            <v-row>
                                                                <v-col cols="12" sm="12" md="12">
                                                                    <v-text-field v-model="editedItem.email" variant="underlined" :rules="emailRules"
                                                                        label="E-mail"></v-text-field>
                                                                </v-col>
                                                            </v-row>
                                                            <v-row>
                                                                <v-col cols="12" sm="12" md="12">
                                                                    <v-text-field v-model="editedItem.phone" variant="underlined" :rules="phoneRules"
...
export default {
    ...
    data: () => ({
        ...
        phoneRules: [ (v) => v?.length > 9 && /[0-9-]+/.test(v) || "Phone Number is required and must be 10 digits",],
    ...
    methods: {
        ...
        deleteItemConfirm() {
            this.dataForm.delete(route('customer.destroy',this.editedItem.id), {
                preserveScroll: true,
            });
            this.closeDelete()
        },
        ...
        async save() {
            ...
            this.dataForm.email = this.editedItem.email;
            this.dataForm.phone = this.editedItem.phone;
            const { valid } = await this.$refs.form.validate();
            if (valid){
                this.error = "";
                if(this.editedItem.id === ''){
                    this.dataForm.post(route('customer.store'), {
                        preserveScroll: true,
                        onSuccess: () => {
                            this.dataForm.reset();
                        },
                ...
                {
                    this.dataForm.put(route('customer.update',this.dataForm.id), {
                        preserveScroll: true,
                        onSuccess: () => {
                            this.dataForm.reset();
                        },
